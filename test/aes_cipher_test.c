...
void test_inv_sub_bytes(void) {
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ASSERT_EQ(s[i][j], inv_sbox[4*i + j]);
        }
    }
    free_smatrix(&s);
}
...
void test_inv_mix_columns(void) {
    smatrix s = alloc_smatrix();

    s[0][0] = 0x04; s[1][0] = 0x66; s[2][0] = 0x81; s[3][0] = 0xe5;
